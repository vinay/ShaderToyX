...
GLuint renderer_get_buffer_texture(Renderer *r, int buf_index)
{
    if (buf_index >= 0 && buf_index < NUM_BUFFERS)
    {
        /* Read from the side we last wrote to (current ^ 1 after swap,
           but we call this AFTER swap, so current IS the readable one) */
...
GLuint renderer_get_buffer_fbo(Renderer *r, int buf_index)
{
    if (buf_index >= 0 && buf_index < NUM_BUFFERS)
    {
        /* Write to the OTHER side from what's readable */
